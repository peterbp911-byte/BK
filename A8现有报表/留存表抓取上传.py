...
yesterday = time.localtime(time.time() - 2*24 * 3600)
sinceDay = time.localtime(time.time() - 32 * 24 * 3600)
...
TABLE_CONFLICT_COLUMNS = {
    "A8_ReRetaintion": ["statisDate"],
    "M9_ReRetaintion": ["statisDate"],
    "T1_ReRetaintion": ["statisDate"],
    "A8_BetRetaintion": ["statisDate"],
    "M9_BetRetaintion": ["statisDate"],
    "T1_BetRetaintion": ["statisDate"]
}
...
URL_TABLE_MAP = {
    ...
    "ReRetaintion_url": "ReRetaintion",
    "BetRetaintion_url": "BetRetaintion",
}
...
def upload_to_db(
    ...
    if conflict_cols is None:
        ...
        if not conflict_cols and table_name in TABLE_CONFLICT_COLUMNS:
            print(f"⚠️ {plat} 目标表 {table_name} 有冲突定义但未找到冲突列，请检查 TABLE_CONFLICT_COLUMNS")
    ...
    if conflict_cols:
        ...
        if valid_conflict_cols:
            ...
            update_columns = [col for col in columns if col not in valid_conflict_cols]
            if update_columns:
                ...
                ).format(table_identifier, columns_sql, values_sql, conflict_sql, update_sql)
            else:
                insert_sql = sql.SQL(
...
def main(plat: str, startTime: int, endTime: int):
    # iterate over the two retention endpoints and upload each
    keys = ['ReRetaintion_url', 'BetRetaintion_url']
    for key in keys:
        try:
