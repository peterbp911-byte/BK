...
# 11个脚本任务
SCRIPTS = [
    {"name": "用户层级-新", "script": "C:/Users/wsmian/Desktop/用户资金-流失/用户资金-流失/用户层级/用户层级-新.py","time": (11,16)},
    {"name": "短信回流_日周汇总", "script": "C:/Users/wsmian/Desktop/用户资金-流失/用户资金-流失/用户层级/短信回流_日周汇总.py","time": (11,52)} ,

    {"name": "用户层级-旧", "script": "C:/Users/wsmian/Desktop/用户资金-流失/用户资金-流失/用户层级/用户层级-旧.py","time": (12,25)},
{"name": "兑换码", "script": "C:/Users/wsmian/Desktop/用户资金-流失/用户资金-流失/用户层级/兑换码.py","time": (12,40)},
{"name": "站内信", "script": "C:/Users/wsmian/Desktop/用户资金-流失/用户资金-流失/用户层级/站内信.py","time": (12,40)}
   ]
...
def execute_script(script_name, script_path):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if result.stderr:
                # 过滤掉 InsecureRequestWarning，只保留真正的错误
                stderr_lines = [l for l in result.stderr.splitlines() if
                                'InsecureRequestWarning' not in l and 'warnings.warn' not in l]
                filtered_stderr = '\n'.join(stderr_lines).strip()
                if filtered_stderr:
                    logger.error(f"错误信息:\n{filtered_stderr[:3000]}")
            if result.stdout:
                stdout_tail = '\n'.join(result.stdout.splitlines()[-20:])
                logger.error(f"脚本输出(末尾):\n{stdout_tail[:2000]}")
            return {"name": script_name, "status": "❌ 失败", "duration": duration, "returncode": result.returncode}
    
    except subprocess.TimeoutExpired:
        logger.error(f"⏱ {script_name} 超时 (2小时)")
        return {"name": script_name, "status": "⏱ 超时", "duration": 7200, "returncode": -1}
    except Exception as e:
...
def wait_for_tasks():
    ...
    while True:
        ...
        # 执行任务
        if not executed_today and (current_time == time_a ):
